...
class Point:
    ...
    def distFrom(self, other):
        ...
        if self.dim != other.dim:
            raise ValueError("dimension mismatch: self has dim {} and other has dim {}".format(self.dim, other.dim))
        
        # fill in
        else:
          for i in range(len(self.coords)):
            dist = (self.coords[i] - other.coords[i])**2 + dist
          dist = math.sqrt(dist)
...
def makePointList(data):
    ...
    pointCoords = []
    for i in range(rows):
      pointCoords.append(Point(data[i][:]))
